...
function StockCardComponent({ data, companyName, peRatio }) {
    // Memoize price calculations
    const latestPrice = useMemo(() => data.price_data?.at(-1)?.close ?? 0, [data.price_data]);
    const previousPrice = useMemo(() => data.price_data?.at(-2)?.close ?? latestPrice, [data.price_data, latestPrice]);
    const priceChange = useMemo(() => latestPrice - previousPrice, [latestPrice, previousPrice]);
    const priceChangePercent = useMemo(() => previousPrice !== 0 ? ((priceChange / previousPrice) * 100).toFixed(2) : "0.00", [previousPrice, priceChange]);
    const isPositive = useMemo(() => priceChange >= 0, [priceChange]);
    const totalVolume = useMemo(() => data.price_data?.at(-1)?.volume ?? 0, [data.price_data]);
    ...
    const sparklineData = useMemo(() => data.price_data?.slice(-20).map(p => p.close) ?? [], [data.price_data]);
    ...
    const volatility = useMemo(() => sparklineData.length > 0 ? calculateVolatility(sparklineData) : "Média", [sparklineData]);
    ...
    return (_jsxs(Card, { className: "rounded-lg sm:rounded-2xl shadow-md hover:shadow-xl hover:scale-105 transition-all duration-300 dark:bg-slate-900 dark:border-slate-800/60 dark:shadow-lg dark:hover:shadow-2xl animate-in fade-in duration-500", children: [_jsxs(CardHeader, { className: "flex flex-col sm:flex-row items-start justify-between pb-2 sm:pb-3 gap-2", children: [_jsxs("div", { className: "flex-1 min-w-0", children: [_jsx(CardTitle, { className: "text-xl sm:text-2xl font-bold truncate dark:text-slate-50", children: data.ticker }), companyName && (_jsx("p", { className: "text-xs sm:text-sm text-muted-foreground dark:text-slate-400 truncate", children: companyName }))] }), _jsx(Tooltip, { text: predictionContent.tooltip, position: "left", children: _jsxs(Badge, { className: `${predictionContent.color} ${predictionContent.textColor} gap-1 sm:gap-2 px-2 sm:px-3 py-1 sm:py-2 uppercase font-bold text-[10px] sm:text-xs cursor-help flex-shrink-0`, children: [predictionContent.icon, _jsx("span", { className: "hidden sm:inline", children: predictionContent.label }), _jsx("span", { className: "sm:hidden", children: data.prediction })] }) })] }), _jsxs(CardContent, { className: "space-y-3 sm:space-y-5", children: [_jsxs("div", { children: [_jsxs("div", { className: "text-3xl sm:text-4xl font-extrabold dark:text-slate-50", children: ["$", latestPrice.toFixed(2)] }), _jsxs("div", { className: `flex items-center gap-1 text-xs sm:text-sm font-semibold ${isPositive ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`, children: [isPositive ? _jsx(TrendingUp, { size: 14 }) : _jsx(TrendingDown, { size: 14 }), isPositive ? "+" : "", "$", priceChange.toFixed(2), " (", priceChangePercent, "%)"] }), sparklineData.length > 0 && (_jsxs("div", { className: "mt-2 sm:mt-3 bg-muted/30 dark:bg-slate-800/70 rounded-lg p-2", children: [_jsxs("div", { className: "text-[11px] sm:text-xs text-muted-foreground dark:text-slate-400 mb-1", children: ["\u00DAltimos ", sparklineData.length, " dias"] }), _jsx(Sparkline, { data: sparklineData, width: 200, height: 40 })] }))] }), _jsxs("div", { className: "space-y-1.5", children: [_jsxs("div", { className: "flex justify-between items-center gap-2", children: [_jsx(Tooltip, { text: `Nível de certeza: ${confidenceValue >= 80 ? 'Alto' : confidenceValue >= 60 ? 'Médio' : 'Baixo'}`, children: _jsx("p", { className: "text-xs sm:text-sm font-semibold text-muted-foreground dark:text-slate-400 cursor-help", children: "Confian\u00E7a" }) }), _jsxs("span", { className: "text-xs sm:text-sm font-bold dark:text-slate-50", children: [confidenceValue.toFixed(1), "%"] })] }), _jsx("div", { className: "w-full bg-secondary dark:bg-slate-700/70 rounded-full h-2 sm:h-3 overflow-hidden", children: _jsx("div", { className: `h-full ${confidenceColor} transition-all duration-500`, style: { width: `${confidenceValue}%` } }) })] }), _jsxs("div", { className: "space-y-1.5", children: [_jsx("p", { className: "text-[10px] sm:text-xs font-semibold uppercase text-muted-foreground", children: "Posicionamento" }), _jsxs("div", { className: "relative h-10 sm:h-12 rounded-lg overflow-hidden bg-muted dark:bg-slate-700 flex", children: [_jsx("div", { className: "w-[40%] bg-rose-500/30 dark:bg-rose-500/40" }), _jsx("div", { className: "w-[20%] bg-amber-500/30 dark:bg-amber-500/40" }), _jsx("div", { className: "w-[40%] bg-emerald-500/30 dark:bg-emerald-500/40" }), _jsx("div", { className: "absolute top-1/2 -translate-y-1/2 transition-all duration-300", style: { left: `${predictionPosition}%` }, children: _jsxs("div", { className: "flex flex-col items-center -translate-x-1/2", children: [_jsx("div", { className: `w-0 h-0 border-l-6 sm:border-l-8 border-r-6 sm:border-r-8 border-b-[8px] sm:border-b-[10px] border-l-transparent border-r-transparent ${data.prediction === "BUY"

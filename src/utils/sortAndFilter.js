...
// Função de ordenação
export const sortStocks = (stocks, sortBy) => {
    const sorted = [...stocks];
    switch (sortBy) {
        case 'ticker-asc':
...
// Função de filtro
export const filterStocks = (stocks, filterBy) => {
    if (filterBy === 'all') {
        return stocks;
    }
    return stocks.filter(stock => stock.data.prediction.toUpperCase() === filterBy.toUpperCase());
};
